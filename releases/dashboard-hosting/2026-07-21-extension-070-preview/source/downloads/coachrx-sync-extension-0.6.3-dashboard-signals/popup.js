...
async function init() {
  const settings = await chrome.storage.sync.get(["webAppUrl", "syncSecret", "selectedCoachId", "selectedCoachName", "coachOptions"]);
...
async function testCoachRx() {
  await runWithUi(async () => {
    ...
    writeLog([
      ...
      `Clients lus: ${data.clients.length}`,
...
async function syncNow() {
  await runWithUi(async () => {
    ...
    const webAppUrl = settings.webAppUrl || webAppUrlInput.value.trim();
    ...
    if (data.coachId && String(data.coachId) !== String(selectedCoachId)) {
...
async function scanAdvancedCoachRx() {
  await runWithUi(async () => {
    ...
    if (scan.coachId && String(scan.coachId) !== String(selectedCoachId)) {
...
function scanCoachRxAdvancedInPage() {
  ...
  function findCoachId(url) {
    ...
    const linkMatch = links.map((link) => link.href.match(/\/team\/(\d+)/i)).find(Boolean);
...
async function fetchCoachRxDataInPage() {
  try {
    const coachId = findCoachId();
    if (!coachId) {
      throw new Error("Coach ID introuvable dans l URL. Va dans CoachRx > Clients pour le coach vise.");
    }
    const apiData = await fetchCoachRxClientsFromApi(coachId);
    const clients = normalizeClients(apiData.clients || []);
    ...
    try {
      ...
      if (!visibleClients.length) throw apiError;
      return {
        ok: true,
        coachId: findCoachId() || "",
        sourceMode: "coachrx-visible-page",
        sourcePath: window.location.href,
        apiError: String(apiError && apiError.message ? apiError.message : apiError),
        clients: visibleClients
      };
    } catch (visibleError) {
      return { ok: false, error: String(visibleError && visibleError.message ? visibleError.message : visibleError) };
    }
  }
  ...
  function findCoachId() {
    ...
    const links = Array.from(document.querySelectorAll("a[href*='/team/'][href*='/clients']"));
    const linkMatch = links.map((link) => link.href.match(/\/team\/(\d+)\/clients/i)).find(Boolean);
    return linkMatch ? linkMatch[1] : "";
  }

  async function fetchCoachRxClientsFromApi(coachId) {
    async function readJson(path) {
      ...
      try {
        ...
        throw new Error(`CoachRx a retourne du non JSON (${response.status}): ${text.slice(0, 180)}`);
      }
      if (!response.ok) throw new Error(`${response.status}: ${JSON.stringify(json).slice(0, 240)}`);
      return json;
    }

    const base = `/api/v1/coaches/${coachId}/clients.json`;
    const candidates = [`${base}?per_page=200`, `${base}?page=1&per_page=200`, base];
    let best = { clients: [], sourcePath: "" };
    for (const candidate of candidates) {
      const data = await readJson(candidate);
      const clients = Array.isArray(data.clients) ? data.clients : Array.isArray(data) ? data : [];
      if (clients.length > best.clients.length) best = { ...data, clients, sourcePath: candidate };
    }

    const byId = new Map(best.clients.map((client) => [String(client.id || client.client_id || client.name), client]));
    for (let pageNumber = 1; pageNumber <= 10; pageNumber += 1) {
      const data = await readJson(`${base}?page=${pageNumber}&per_page=100`);
      const pageClients = Array.isArray(data.clients) ? data.clients : Array.isArray(data) ? data : [];
      if (!pageClients.length) break;
      pageClients.forEach((client) => byId.set(String(client.id || client.client_id || client.name), client));
      if (pageClients.length < 100) break;
    }

    return { ...best, clients: Array.from(byId.values()) };
  }
  ...
  function normalizeClients(clients) {
    return clients.map((client) => {
      ...
      return {
        ...
        id: stringValue(firstValue(client.id, client.client_id)),
