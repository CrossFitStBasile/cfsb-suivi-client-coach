...
async function fetchCoachRxRawDataInMainWorld(expectedCoachId) {
  try {
    ...
    let sourcePath = `${base}?page=1&per_page=100`;
    let pageCount = 0;

    for (let pageNumber = 1; pageNumber <= 25; pageNumber += 1) {
      const path = `${base}?page=${pageNumber}&per_page=100`;
      const data = await readJson(path);
      const pageClients = clientsFrom(data);
      pageCount += 1;
      pageClients.forEach((client, index) => {
        const identity = String(
          ...
          || client?.contact_id
          || client?.contactId
          || `${pageNumber}:${index}:${client?.name || client?.full_name || "unknown"}`
        );
        byIdentity.set(identity, client);
      });
      if (!pageClients.length || pageClients.length < 100) break;
    }

    if (!byIdentity.size) {
      const fallbackPath = `${base}?per_page=200`;
      const data = await readJson(fallbackPath);
      clientsFrom(data).forEach((client, index) => {
        const identity = String(client?.id || client?.client_id || client?.uuid || `fallback:${index}`);
        byIdentity.set(identity, client);
      });
      sourcePath = fallbackPath;
    }
    if (!byIdentity.size) throw new Error("Aucun client retourne par l API CoachRx.");
    ...
    return {
      ...
      sourcePath,
      pageCount,
      clients: Array.from(byIdentity.values())
    };
  } catch (error) {
    return {
      ok: false,
