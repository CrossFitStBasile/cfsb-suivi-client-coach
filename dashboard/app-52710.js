...
const QUESTIONNAIRE_KNOWN_KEYS = new Set([
  "response_id", "responseid", "id", "submitted_at", "submittedat", "received_at", "receivedat",
  "client_name", "clientname", "client", "client_email", "clientemail", "client_phone", "clientphone",
  "client_phone_normalized", "clientphonenormalized", "client_key", "clientkey", "client_id", "clientid",
  "coach_name", "coachname", "coach", "followup_type", "followuptype", "general_state", "generalstate",
  "motivation_level", "motivationlevel", "goal_status", "goalstatus", "goal_change_detail", "goalchangedetail",
  "goal_clarity_score", "goalclarityscore", "progress_toward_goal", "progresstowardgoal",
  "recent_success_type", "recentsuccesstype", "recent_success", "recentsuccess", "last_30_days_attendance",
  "last30daysattendance", "results_satisfaction_score", "resultssatisfactionscore", "current_challenges",
  "currentchallenges", "program_fit", "programfit", "program_adjustment_detail", "programadjustmentdetail",
  "pain_status", "painstatus", "pain_detail", "paindetail", "support_needed", "supportneeded",
  "contact_request", "contactrequest", "open_note", "opennote", "note", "improvements_requested",
  "improvementsrequested", "final_position", "finalposition", "response_source_tab", "sourcetab", "source",
  "triage_status", "triagestatus", "coach_action_type", "coachactiontype", "processing_status",
  "processingstatus", "dashboard_sync_status", "dashboardsyncstatus", "match_status", "matchstatus",
  "dashboard_match_status", "dashboardmatchstatus", "raw_payload_json", "rawpayloadjson"
]);
...
function taskActionButtons(task) {
  ...
  if (type === "formulaire") {
    const formText = normalizeLower([task.action, task.why, task.due, task.source].filter(Boolean).join(" "));
    const formLabel = formText.includes("relancer") || formText.includes("sans_reponse")
      ? "Relancer"
      : "Envoyer questionnaire";
    return `<div class="row-actions">
      ${questionnaireSendButton(`data-send-questionnaire="true" ${clientAttrs}`, false, formLabel)}
      <button ${baseAttrs} data-status="Ignore">Masquer</button>
    </div>`;
  }
  if (type === "validation") {
  ...
  return `
    <aside class="client-focus">
      ...
      <div class="focus-note client-management" data-client-key="${escapeAttr(client.clientKey || "")}" data-client-name="${escapeAttr(client.client || "")}">
        ...
        <p>Modifier les informations utiles ou classer le client s'il ne doit plus apparaitre dans la liste active.</p>
        <div class="form-grid compact-form">
        ...
        <div class="actions-row">
          <button id="editClientBtn">Modifier la fiche</button>
    ...
    <section class="mission-panel performance-hero">
      <div class="command-hero">
        <div>
          ...
          <p>Cette section regroupe les seances a remettre pour le coach selectionne. Les statuts se gerent ici; l'app externe reste disponible comme vue de detail et filet de securite.</p>
        </div>
        <div class="hero-actions">
          ${rebookingUrl ? `<a class="button-link secondary-link" href="${escapeAttr(rebookingUrl)}" target="_blank" rel="noopener noreferrer">Ouvrir app externe</a>` : `<span class="freshness">Lien a configurer</span>`}
...
function renderQuestionnaireInbox(data) {
  ...
  const unread = activeResponses.filter((row) => row.matchStatus !== "non_matche" && !["lu", "action_completee", "archive"].includes(row.processingStatus));
  ...
  const primaryListHtml = state.activeQuestionnaireFilter === "send"
    ? (sendableClients.length ? `<div class="questionnaire-list">${sendableClients.map((client) => questionnaireClientRow(client, latestQuestionnaireSendForClient(sends, client))).join("")}</div>` : '<div class="empty">Aucun client disponible.</div>')
    : state.activeQuestionnaireFilter === "relance"
      ...
      : state.activeQuestionnaireFilter === "archive"
        ...
        : (displayRows.length ? `<div class="questionnaire-list">${displayRows.map(questionnaireRow).join("")}</div>` : '<div class="empty">Aucune reponse a lire.</div>');
  ...
  els.content.innerHTML = `
    <section class="mission-panel performance-hero">
      ...
      <div class="priority-strip">
        ${priorityTile("A lire", unread.length, "p2")}
...
function questionnaireStatusNotice() {
  ...
  const body = QUESTIONNAIRE_LIVE_ENABLED
    ? "Le bouton Envoyer questionnaire ajoute le tag GHL dashboardcoach au contact trouve par telephone. Une confirmation apparait avant l'envoi."
...
function normalizeQuestionnaireSendLog(row) {
  ...
  const phone = cleanValue(row.phone || row.Phone || row.clientPhone || row.client_phone || row.Telephone || row["Telephone"]);
...
function latestQuestionnaireSendForClient(sends, client) {
  const clientKey = cleanValue(client && client.clientKey);
  const clientName = normalizeLower(client && client.client);
  const clientPhone = normalizePhoneDigits(client && client.phone);
  return (sends || [])
    .filter((row) => {
      const rowPhone = normalizePhoneDigits(row.phone || row.clientPhone || row.client_phone || row.phoneNormalized || row.client_phone_normalized);
      return (clientKey && row.clientKey === clientKey)
        || (clientName && normalizeLower(row.clientName) === clientName)
        || (clientPhone && rowPhone && rowPhone === clientPhone);
    })
    .sort((a, b) => sortableDate(b.sentAt || b.createdAt || "") - sortableDate(a.sentAt || a.createdAt || ""))[0] || null;
}
...
function isQuestionnaireSentWithoutResponse(row) {
  ...
  return status.includes("envoye") || status.includes("envoyé") || status.includes("a_suivre") || status === "";
}
...
function normalizeQuestionnaireResponse(row) {
  ...
  return {
    ...
    clientKey,
    otherAnswers: questionnaireOtherAnswers(row)
  };
}

function questionnaireOtherAnswers(row) {
  return Object.keys(row || {})
    .filter((key) => {
      const value = row[key];
      if (value === null || value === undefined || cleanValue(value) === "") return false;
      const normalizedKey = normalizeQuestionnaireKey(key);
      if (QUESTIONNAIRE_KNOWN_KEYS.has(normalizedKey)) return false;
      if (normalizedKey.includes("raw") || normalizedKey.includes("payload") || normalizedKey.includes("json")) return false;
      if (typeof value === "object") return false;
      return true;
    })
    .map((key) => ({
      label: humanizeQuestionnaireKey(key),
      value: cleanValue(row[key])
    }))
    .slice(0, 12);
}

function normalizeQuestionnaireKey(key) {
  return String(key || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
function humanizeQuestionnaireKey(key) {
  return String(key || "")
    .replace(/[_-]+/g, " ")
...
function questionnaireFilters(responses, staleClients, sentNoResponse, relanceDue, sendErrors, clients) {
  ...
  const filters = [
    ["all", "A lire", unread.length],
    ["send", "Envoyer", clients.length],
    ["relance", "A relancer", relanceDue.length],
  ...
  return `
    ...
    <p class="filter-help">Choisis l'action: lire une reponse, envoyer un questionnaire, relancer apres 7 jours sans reponse, valider un non-match ou consulter les archives. Les couleurs servent seulement de signal de priorite dans les cartes.</p>
    ${sendErrors.length ? `<div class="status-note bad"><strong>${sendErrors.length} erreur${sendErrors.length > 1 ? "s" : ""} GHL</strong><span>Va dans A relancer pour voir les derniers envois en erreur.</span></div>` : ""}
  `;
}

function questionnaireFilterTitle(filtered, staleClients) {
  if (state.activeQuestionnaireFilter === "send") return "Envoyer un questionnaire";
  ...
  return `${filtered.length} reponse${filtered.length > 1 ? "s" : ""} a lire`;
}
...
function filterQuestionnaireResponses(responses) {
  ...
  return activeResponses
    .filter((row) => row.matchStatus !== "non_matche")
...
function questionnaireRow(row) {
  ...
  const otherAnswers = row.otherAnswers || [];
  return `
    <article class="compact-row questionnaire-row">
      ...
      ${otherAnswers.length ? `<details class="answer-details"><summary>Autres reponses (${otherAnswers.length})</summary>${otherAnswers.map((item) => `<p><strong>${escapeHtml(item.label)}:</strong> ${escapeHtml(item.value)}</p>`).join("")}</details>` : ""}
...
function questionnaireClientRow(client, lastSend) {
  ...
  const sendInfo = lastSend ? questionnaireSendSummary(lastSend) : "Aucune tentative d'envoi connue";
  return `
    <article class="compact-row questionnaire-row">
      <strong>${escapeHtml(client.client || "Client")}</strong>
      ...
      <p class="muted compact-meta">${escapeHtml(sendInfo)}</p>
      <div class="row-actions compact-actions">
...
function questionnaireSendSummary(row) {
  if (!row) return "";
  if (isQuestionnaireSendError(row)) return `Derniere tentative: erreur GHL${row.note ? " - " + row.note : ""}`;
  if (isQuestionnaireSentWithoutResponse(row)) {
    const days = Number.isFinite(Number(row.daysSinceSent)) ? `, ${row.daysSinceSent} j sans reponse` : "";
    return `Dernier envoi confirme: ${row.sentAt || "date inconnue"}${days}`;
  }
  return `Derniere tentative: ${row.status || "statut a verifier"}${row.sentAt ? " - " + row.sentAt : ""}`;
}
...
function questionnaireSendLogRow(row) {
  ...
  return `
    <article class="compact-row questionnaire-row">
      ...
      <div class="row-actions compact-actions">
        ${row.clientName ? questionnaireSendButton(`data-send-questionnaire="true" data-client-key="${escapeAttr(row.clientKey || "")}" data-client-name="${escapeAttr(row.clientName || "")}"`, false, isError ? "Reessayer" : "Relancer") : ""}
      </div>
    </article>
  `;
}

function questionnaireSendButtonLabel(labelOverride) {
...
function questionnaireSendButton(attrs, disabled, labelOverride) {
  ...
  return `<button class="${classes}" ${attrs} ${disabled || livePaused ? "disabled" : ""} ${title}>${questionnaireSendButtonLabel(labelOverride)}</button>`;
}
...
function renderScenarios(data) {
  ...
  els.content.innerHTML = `
    ...
    ${state.isDemo ? `
      <section class="mission-panel performance-hero">
        <div class="section-head"><div><p class="eyebrow">Mode demo</p><h2>Scenarios coach a tester</h2></div></div>
        <p class="muted">Ces cartes existent seulement en mode demo. Elles ne sont pas affichees aux coachs en utilisation reelle.</p>
        ${demoBanner()}
    ...
    ` : `
      <section class="mission-panel performance-hero">
        <div class="section-head"><div><p class="eyebrow">Support</p><h2>Modules actifs</h2></div></div>
        <div class="goal-grid">
          <article class="compact-row"><strong>Usage quotidien</strong><p>To-do, Clients, Rebooking, Questionnaires, Performance et Alumni sont les zones de travail reelles.</p></article>
          <article class="compact-row"><strong>Mode demo cache</strong><p>Les scenarios de test restent disponibles dans Systeme pour l'administration, pas dans le flux coach.</p></article>
        </div>
      </section>
    `}
    <section class="mission-panel performance-hero">
      ...
      <div class="goal-grid">
        <article class="compact-row"><strong>To-do</strong><p>Les taches doivent apparaitre avec les bons boutons: Fait, Masquer, Rebooke, Absence coach, Gere, Relancer ou Envoyer selon le contexte.</p></article>
  ...
  return [
    {
      ...
      status: "Actif pilote",
      tone: "good",
      description: "Interface visuelle client-coach. Le dashboard peut envoyer le tag GHL dashboardcoach, lire les reponses et classer les suivis dans l'inbox privee.",
      links: [
    ...
    },
    {
      ...
      status: "Integre pilote",
      tone: "good",
      description: activeRebookingUrl
        ? "Les dossiers ouverts et l'historique sont visibles dans l'onglet Rebooking. L'app externe reste disponible en vue de detail."
        : "Lien app rebooking a configurer pour ce coach. L'onglet Rebooking reste la vue principale.",
      links: activeRebookingUrl ? [["Ouvrir app externe", activeRebookingUrl]] : []
    },
    {
  ...
  els.content.innerHTML = `
    <section class="mission-panel performance-hero security-panel">
      ...
      <div class="focus-facts security-facts">
        ...
        ${focusFact("Ecritures", "Apps Script protege par PIN")}
        ${focusFact("Deploiement", "Pilote controle")}
      </div>
      <div class="focus-note"><strong>Decision a moyen terme</strong><p>Le pilote est verrouille par PIN. Avant un deploiement large, il faudra ajouter une vraie couche d'authentification par coach.</p></div>
    ...
    <section class="mission-panel">
      ...
      <article class="compact-row"><strong>Backend actions</strong><p>Continuer a optimiser les ecritures Apps Script et reduire les reconstructions completes quand une action simple est faite.</p></article>
      <article class="compact-row"><strong>Inbox questionnaire</strong><p>Surveiller les vrais cas GHL: envoye, erreur, contact introuvable, relance apres 7 jours et reponse recue.</p></article>
      <article class="compact-row"><strong>Fiche client</strong><p>Continuer a enrichir l'historique client sans surcharger la fiche quotidienne.</p></article>
    </section>
  `;
}
...
async function updateTask(taskId, rowNumber, status, button) {
  ...
  if (["Fait", "Ignore"].includes(status) && taskId) {
    const restoreBusy = setButtonBusy(button, status === "Fait" ? "Note..." : "Masque...");
...
async function sendQuestionnaire(clientKey, clientName, taskId, button) {
  ...
  if (!phoneDigits) {
    showToast("Telephone manquant: ajoute un numero avant d'envoyer le questionnaire.", true);
    return;
  }
  const confirmed = window.confirm([
    ...
    `Telephone: ${phone}`,
    `Coach: ${state.activeCoach || "Coach a confirmer"}`,
    "",
    "Le contact sera valide par telephone dans GoHighLevel, puis le tag dashboardcoach sera ajoute."
  ].join("\n"));
  if (!confirmed) return;
  ...
  try {
    ...
    if (latest && isQuestionnaireSendError(latest)) {
      showToast(`Erreur GHL: ${questionnaireSendErrorMessage(latest)}`, true);
      ...
      showToast("Tentative journalisee. Verifie la vue Questionnaires si le SMS n'arrive pas.");
    }
    render();
  } catch (error) {
    renderError(error);
...
function questionnaireSendErrorMessage(row) {
  const note = cleanValue(row && row.note);
  const status = cleanValue(row && row.status);
  const text = normalizeLower([note, status].join(" "));
  if (text.includes("introuv") || text.includes("not_found") || text.includes("not found")) return "contact introuvable par telephone dans GoHighLevel.";
  if (text.includes("phone") || text.includes("telephone") || text.includes("tÃ©lÃ©phone")) return "telephone invalide ou manquant dans GoHighLevel.";
  return note || status || "erreur non detaillee retournee par GoHighLevel.";
}
