  ...
  return `
    <aside class="client-focus">
      ...
      <div class="focus-note client-management" data-client-key="${escapeAttr(client.clientKey || "")}" data-client-name="${escapeAttr(client.client || "")}">
        <strong>Classement du client</strong>
        <p>Utilise cette zone seulement si le client ne doit plus apparaitre dans le portefeuille actif du coach.</p>
        <div class="form-grid compact-form">
        ...
        <div class="actions-row">
          ...
          <button id="removeClientBtn">Appliquer le classement</button>
        </div>
      </div>
...
function questionnaireStatusNotice() {
  ...
  const body = QUESTIONNAIRE_LIVE_ENABLED
    ? "Le bouton Envoyer questionnaire ajoute le tag GHL dashboardcoach au contact trouve par telephone. Si GHL ne trouve pas le contact ou refuse l'action, l'erreur reste visible ici."
...
function questionnaireFilters(responses, staleClients, sentNoResponse, relanceDue, sendErrors, clients) {
  ...
  const filters = [
    ["all", "Reponses a lire", unread.length],
    ["send", "Envoyer a un client", clients.length],
    ["relance", "Relances dues", relanceDue.length + sendErrors.length],
  ...
  return `
    ...
    <p class="filter-help">Les couleurs dans les cartes indiquent l'urgence. Les onglets gardent seulement les vraies decisions coach: lire, envoyer, relancer, valider ou archiver.</p>
...
function questionnaireFilterTitle(filtered, staleClients) {
  if (state.activeQuestionnaireFilter === "send") return "Choisir un client";
...
async function sendQuestionnaire(clientKey, clientName, taskId, button) {
  ...
  try {
    ...
    showToast(`Questionnaire non envoye: ${error.message || String(error)}`, true);
  } finally {
    restoreBusy();
  }
}
