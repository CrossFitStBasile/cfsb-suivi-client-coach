...
function renderQuestionnaireInbox(data) {
  ...
  const primaryListHtml = state.activeQuestionnaireFilter === "stale"
  ...
  els.content.innerHTML = `
    <section class="mission-panel performance-hero">
      ...
      ${questionnaireFilters(responses, staleClients, pending)}
    </section>
    <section class="mission-panel performance-hero">
...
function questionnaireFilters(responses, staleClients, pending) {
  const filters = [
  ...
  return `
    <article class="compact-row questionnaire-row">
      ...
      <div class="row-actions compact-actions">
        <button class="done" data-send-questionnaire="true" data-client-key="${escapeAttr(client.clientKey || "")}">Envoyer questionnaire</button>
      </div>
    </article>
  `;
}
...
function renderAlumni(v3) {
  ...
  els.content.innerHTML = `
    ...
    <section class="mission-panel">
      ...
      ${rows.length ? rows.map((row) => `<article class="compact-row"><strong>${escapeHtml(row.client)} <span class="tag">${escapeHtml(row.status || "Alumni")}</span></strong><p>Prochain contact: ${escapeHtml(row.nextContactDue || "A planifier")}</p><p>${escapeHtml(row.notes || "")}</p><div class="row-actions compact-actions"><button data-create-alumni-task="true" data-client="${escapeAttr(row.client || "")}">Creer mission</button></div></article>`).join("") : '<div class="empty">Aucun alumni entre.</div>'}
    </section>
    <section class="mission-panel">
      <div class="section-head"><div><p class="eyebrow">Retires / exclus</p><h2>A ne pas recontacter</h2></div></div>
      ${removed.length ? removed.map((row) => `<article class="compact-row"><strong>${escapeHtml(row.client)} <span class="tag">${escapeHtml(row.status || "Retire")}</span></strong><p>${escapeHtml(row.notes || "")}</p></article>`).join("") : '<div class="empty">Aucun client exclu pour le moment.</div>'}
    </section>`;
...
function renderScenarios(data) {
  ...
  els.content.innerHTML = `
    <section class="mission-panel performance-hero">
      <div class="command-hero">
        <div>
          <p class="eyebrow">Banc d'essai</p>
          <h2>Scenarios coach a tester</h2>
          <p>Chaque carte cree ou replace un cas concret dans le mode demo. Ca sert a valider les workflows avant de les brancher a de vrais clients.</p>
        </div>
        <span class="freshness">${state.isDemo ? "Demo active" : "Active le mode demo"}</span>
      </div>
    ...
    <section class="mission-panel performance-hero">
      ...
      <div class="goal-grid">
        ...
        <article class="compact-row"><strong>Alumni / Performance</strong><p>Les anciens clients, les holds et les impacts doivent nourrir les bonnes vues sans polluer le quotidien.</p></article>
      </div>
    </section>
    <section class="mission-panel performance-hero">
...
function demoScenarios() {
  return [
    ...
    },
    {
      ...
      checks: ["Vue Performance", "Rappel avant retour", "Client moins bruyant"],
      button: "Tester hold"
    },
    {
      id: "alumni",
...
function renderPerformance(data) {
  ...
  els.content.innerHTML = `
    ...
    <section class="mission-panel">
      ...
      <div class="actions-row"><button id="saveAlumniBtn" class="primary">Ajouter alumni</button></div>
      ${alumni.length ? alumni.slice(0, 10).map((row) => `<article class="compact-row"><strong>${escapeHtml(row.client)} <span class="tag">${escapeHtml(row.status || "Alumni")}</span></strong><p>Prochain contact: ${escapeHtml(row.nextContactDue || "A planifier")}</p><p>${escapeHtml(row.notes || "")}</p><div class="row-actions compact-actions"><button data-create-alumni-task="true" data-client="${escapeAttr(row.client || "")}">Creer mission</button></div></article>`).join("") : '<div class="empty">Aucun alumni entre.</div>'}
      ${removed.length ? `<div class="section-head mini-head"><div><p class="eyebrow">Retires / exclus</p><h2>A ne pas recontacter</h2></div></div>${removed.slice(0, 8).map((row) => `<article class="compact-row"><strong>${escapeHtml(row.client)} <span class="tag">${escapeHtml(row.status || "Retire")}</span></strong><p>${escapeHtml(row.notes || "")}</p></article>`).join("")}` : ""}
    </section>
    <section class="mission-panel">
  ...
  const saveAlumniButton = document.getElementById("saveAlumniBtn");
  if (saveAlumniButton) saveAlumniButton.addEventListener("click", saveAlumni);
  els.content.querySelectorAll("[data-create-alumni-task]").forEach((button) => {
    button.addEventListener("click", () => createAlumniTask(button.dataset.client || ""));
  });
}
...
function rebuildDemoData(data) {
  ...
  data.v3.kpis = {
    ...
    impactsWeek: ((data.v3.impacts && data.v3.impacts.log) || []).length,
    retentionRate: 92,
...
function demoSaveHold(client) {
  mutateDemoData((data) => {
    ...
    data.v3.holds = data.v3.holds || [];
    data.v3.holds.push({
      clientKey: normalizeName(client),
      client,
...
async function sendQuestionnaire(clientKey, clientName, taskId) {
  ...
  try {
    ...
    state.data = response.result || state.data;
    showToast("Questionnaire envoye via GHL.");
    render();
  } catch (error) {
    renderError(error);
  }
