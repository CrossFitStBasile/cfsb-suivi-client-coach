...
function callApiWithUrl(apiUrl, action, payload) {
  return callApiJsonpWithUrl(apiUrl, action, payload)
    .catch((jsonpError) => callApiFetchWithUrl(apiUrl, action, payload)
      .catch(() => {
        throw jsonpError;
      }));
}
...
function callApiJsonpWithUrl(apiUrl, action, payload) {
  return new Promise((resolve, reject) => {
    ...
    const timer = window.setTimeout(() => {
      cleanup();
      reject(new Error("Le chargement a depasse 25 secondes. Essaie Mettre a jour; si ca revient souvent, il faudra optimiser le backend Apps Script."));
    }, JSONP_TIMEOUT_MS);
    ...
    <section class="mission-panel performance-hero">
      <div class="command-hero">
        <div>
          ...
          <p>Le dashboard lit les donnees privees dans Apps Script. Si ca depasse environ 25 secondes, l'app affichera une erreur au lieu de rester bloquee.</p>
        </div>
...
function startLoading(message) {
  ...
  state.loadingTimer = window.setTimeout(() => {
    if (loadId !== state.loadingSeq) return;
    renderError(new Error("Le backend ne repond pas apres 30 secondes. Tu peux reessayer, verifier le PIN, ou utiliser la demo pendant qu'on optimise Apps Script."));
  }, 30000);
  return loadId;
}
...
function taskActionButtons(task) {
  ...
  if (type === "formulaire") {
    return `<div class="row-actions">
      <button class="done" data-send-questionnaire="true" ${clientAttrs}>${questionnaireSendButtonLabel()}</button>
      <button ${baseAttrs} data-status="Ignore">Masquer</button>
    </div>`;
  }
  if (type === "validation") {
  ...
  return `
    <article class="compact-row questionnaire-row">
      ...
      ${row.responseId ? `<div class="row-actions compact-actions">
        <button data-send-questionnaire-client="true" data-client-key="${escapeAttr(row.clientKey || "")}" data-client-name="${escapeAttr(row.clientName || "")}">${questionnaireSendButtonLabel()}</button>
  ...
  return `
    <article class="compact-row questionnaire-row">
      ...
      <div class="row-actions compact-actions">
        <button class="done" data-send-questionnaire="true" data-client-key="${escapeAttr(client.clientKey || "")}" data-client-name="${escapeAttr(client.client || "")}" ${phone ? "" : "disabled"}>${questionnaireSendButtonLabel()}</button>
      </div>
    </article>
  `;
}
