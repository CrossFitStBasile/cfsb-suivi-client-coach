...
const QUESTIONNAIRE_LIVE_ENABLED = false;
...
const state = {
  ...
  activeView: normalizeView(localStorage.getItem("cfsbCoachView") || "performance"),
...
function render() {
  ...
  const allVisibleTasks = (data.tasks || []).filter((task) => !state.localHiddenTaskIds.has(task.taskId));
  ...
  if (type === "rebooking") {
    if (!state.isDemo) {
      return `<div class="row-actions">
        <button class="pending-action" disabled title="Le workflow GHL de rappel rebooking n'est pas encore branche dans le dashboard.">Rappel a brancher</button>
...
function openClientModal(clientKey) {
  const client = ((state.data && state.data.clients) || []).find((item) => item.clientKey === clientKey);
...
function normalizeView(view) {
  ...
  return map[view] || (["mission", "clients", "questionnaires", "performance", "alumni", "scenarios"].includes(view) ? view : "performance");
}
    ...
    <section class="mission-panel">
      <div class="command-hero">
        <div>
          ...
          <p>Trie les clients selon ce que tu veux traiter: actions, fin de membership, recurrence prevue dans Kilo, questionnaires ou risques coach. Clique un client pour ouvrir sa fiche.</p>
        </div>
...
function questionnaireSendButtonLabel() {
  return state.isDemo ? "Simuler questionnaire" : "Questionnaire en pause";
}
...
function questionnaireSendButton(attrs, disabled) {
  ...
  const classes = state.isDemo ? "done" : "pending-action";
  ...
  els.content.innerHTML = `
    ...
    <section class="mission-panel performance-hero">
      ...
      <div class="goal-grid">
        ...
        <article class="compact-row"><strong>Clients</strong><p>La fiche client doit montrer membership, fin systeme, fin prevue coach, recurrence prevue dans Kilo, risque coach et dernier questionnaire.</p></article>
...
function renderPerformance(data) {
  ...
  const impactsInRange = impacts.filter((row) => dateIsWithinRange(row.impactDate || row.impact_date || row.createdAt || row.created_at, rangeDays));
    ...
    <section class="mission-panel performance-hero">
      <div class="command-hero">
        <div>
          ...
          <p>Cette section sert de premiere page: suivre les indicateurs coach, les risques de perte client et les nouveaux revenus crees.</p>
        </div>
      ...
      <div class="priority-strip">
        ...
        ${priorityTile(`Check-ups`, checkups, "p3")}
...
function performanceRangeButtons() {
  ...
  return `<div class="range-buttons">${ranges.map(([value, label]) => `<button class="${state.performanceRange === value ? "active" : ""}" data-performance-range="${value}">${escapeHtml(label)}</button>`).join("")}</div>`;
}
...
function performanceRangeLabel() {
  const map = { "7": "7j", "30": "30j", "60": "60j", "180": "6m", "365": "12m" };
...
function countClientsInRange(clients, rangeDays) {
  return clients.filter((client) => dateIsWithinRange(client.memberSince || client.startDate || client.membershipStart || client.createdAt, rangeDays)).length;
}
...
function countCheckupsInRange(data, rangeDays) {
  ...
  if (rows.length) {
    return rows.filter((row) => dateIsWithinRange(row.date || row.submitted_at || row.submittedAt || row.checkupDate, rangeDays)).length;
  }
  return (data.clients || []).filter((client) => dateIsWithinRange(client.lastCheckup || client.lastCheckupDate || client.lastConsultation, rangeDays)).length;
}
