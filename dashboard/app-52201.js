  ...
  return `
    <aside class="client-focus">
      ...
      <div class="focus-note client-management" data-client-key="${escapeAttr(client.clientKey || "")}" data-client-name="${escapeAttr(client.client || "")}">
        ...
        <p>Modifier les infos de test ou retirer le client du dashboard.</p>
        <div class="actions-row">
          <button id="editClientBtn">Modifier</button>
          <button id="removeClientBtn">Retirer du dashboard</button>
        </div>
      </div>
    </aside>
  `;
...
function editClientForm(clients) {
  if (!state.editingClientKey) return "";
  const client = (clients || []).find((item) => item.clientKey === state.editingClientKey);
  if (!client) return "";
  return `
    <div class="focus-note manual-client-form">
      <strong>Modifier client demo</strong>
      <p>En mode reel, cette action deviendra une mise a jour controlee. En sandbox, elle modifie seulement tes donnees locales.</p>
      <div class="form-grid compact-form">
...
function clientDirectoryRow(client) {
  const questionnaireLabel = lastQuestionnaireDate(client, client.latestQuestionnaireResponse)
    ? `Questionnaire: ${lastQuestionnaireDate(client, client.latestQuestionnaireResponse)}${questionnaireAgeLabel(client) ? " | " + questionnaireAgeLabel(client) : ""}`
    : "Questionnaire: jamais trouve";
  return `
    <article class="directory-row ${state.selectedClientKey === client.clientKey ? "selected" : ""}">
      ...
      <div class="task-context">
        ...
        ${client.serviceEnd ? `<span class="tag warn">Kilo: ${escapeHtml(client.serviceEnd)}</span>` : ""}
...
function createInitialDemoData() {
  const clients = [
    demoClient("alex-martin", "Alex Martin", "Semi-Prive 2x/sem", -22, 9, "Programme en retard", "62%", 1, "Moyen", "Annule souvent depuis deux semaines"),
...
function rebuildDemoData(data) {
  ...
  data.v3 = Object.assign({ holds: [], alumni: [], impacts: { log: [], opportunities: [] } }, data.v3 || {});
  data.v3.kpis = {
    clients: clients.length,
...
function demoRemoveClient(clientKey, clientName) {
  mutateDemoData((data) => {
    data.clients = (data.clients || []).filter((client) => client.clientKey !== clientKey && client.client !== clientName);
    (data.tasks || []).forEach((task) => {
      if (task.clientKey === clientKey || task.client === clientName) task.status = "Ignore";
    });
    state.selectedClientKey = "";
    state.editingClientKey = "";
  }, "Client demo retire du dashboard.");
}
...
async function removeClientFromDashboard(clientKey, clientName) {
  if (!clientName) return showToast("Client introuvable.");
  if (state.isDemo) {
    demoRemoveClient(clientKey, clientName);
    return;
  }
  try {
