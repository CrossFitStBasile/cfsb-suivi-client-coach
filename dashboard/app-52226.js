...
function clientFocusPanel(client, tasks) {
  ...
  return `
    <aside class="client-focus">
      ...
      <div class="focus-note membership-end-editor">
        <strong>Fin membership manuel</strong>
        <p>Le dashboard ne calcule pas cette date automatiquement. Si une fin d'abonnement est connue, elle doit etre entree ou corrigee manuellement dans la fiche client.</p>
      </div>
...
function operationalModules() {
  return [
    ...
    },
    {
      ...
      status: "Module lie",
      tone: "warn",
      description: "Source utile pour les seances a remettre. Les rappels automatiques aux clients restent a brancher; pour l'instant le dashboard sert surtout a voir et prioriser.",
      links: [
...
function countCheckupsInRange(data, rangeDays) {
  ...
  if (rows.length) {
    return rows
      ...
      .filter((row) => dateIsWithinRange(row.date || row.Date || row.submitted_at || row.submittedAt || row.checkupDate || row["Date"], rangeDays)).length;
  }
