...
document.querySelectorAll(".views button").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    render();
    if (state.activeView === "questionnaires" && !state.isDemo && state.appPin) {
...
document.addEventListener("click", (event) => {
  ...
  const refreshQuestionnairesButton = event.target.closest("[data-refresh-questionnaires]");
  if (refreshQuestionnairesButton) {
    if (refreshQuestionnairesButton.disabled) return;
    refreshQuestionnairesButton.disabled = true;
    loadData(false, true).finally(() => {
      refreshQuestionnairesButton.disabled = false;
    });
  }
...
function questionnaireStatusNotice() {
  ...
  return `<div class="status-note ${tone}"><strong>${escapeHtml(title)}</strong><span>${escapeHtml(body)}</span><button type="button" data-refresh-questionnaires>Actualiser les reponses</button></div>`;
}
...
function normalizeQuestionnaireResponse(row) {
  ...
  return {
    ...
    clientName: cleanValue(row.client_name || row.clientName || row.client_name_entered || row.client),
    clientEmail: cleanValue(row.client_email || row.clientEmail || row.client_email_entered),
    clientPhone: cleanValue(row.client_phone || row.clientPhone || row.client_phone_entered || row.phone),
    clientPhoneNormalized: cleanValue(row.client_phone_normalized || row.clientPhoneNormalized || row.phone_normalized || row.client_phone_entered || row.phone),
    coachName: cleanValue(row.coach_name || row.coachName || row.coach_name_entered || row.coach),
