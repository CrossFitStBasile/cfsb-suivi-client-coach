    ...
    <article class="compact-row rebooking-row">
      ...
      <div class="row-actions compact-actions">
        ${isClosed
          ...
          : `<button class="done" data-rebooking-status="REBOOKE" data-cancellation-id="${escapeAttr(row.cancellationId || "")}" data-client-key="${escapeAttr(clientKey)}" data-client-name="${escapeAttr(row.client || "")}">Rebooke</button>
             <button data-rebooking-status="ABSENCE_COACH" data-cancellation-id="${escapeAttr(row.cancellationId || "")}" data-client-key="${escapeAttr(clientKey)}" data-client-name="${escapeAttr(row.client || "")}">Absence coach</button>
             <button data-rebooking-status="GERE" data-cancellation-id="${escapeAttr(row.cancellationId || "")}" data-client-key="${escapeAttr(clientKey)}" data-client-name="${escapeAttr(row.client || "")}">Gere</button>`}
...
function renderQuestionnaireInbox(data) {
  ...
  const sendErrors = sends.filter((row) => row.statusKey === "erreur");
  ...
  const titleRows = state.activeQuestionnaireFilter === "pending"
    ? sentNoResponse
    : state.activeQuestionnaireFilter === "relance"
      ? relanceDue
      : state.activeQuestionnaireFilter === "errors"
        ? sendErrors
        : filtered;
  const primaryListHtml = state.activeQuestionnaireFilter === "clients"
    ...
    : state.activeQuestionnaireFilter === "stale"
      ...
      : state.activeQuestionnaireFilter === "pending"
        ? (sentNoResponse.length ? sentNoResponse.map(questionnaireSendLogRow).join("") : '<div class="empty">Aucun questionnaire envoye sans reponse.</div>')
      : state.activeQuestionnaireFilter === "relance"
        ? (relanceDue.length ? relanceDue.map(questionnaireSendLogRow).join("") : '<div class="empty">Aucune relance due pour le moment.</div>')
      : state.activeQuestionnaireFilter === "errors"
        ? (sendErrors.length ? sendErrors.map(questionnaireSendLogRow).join("") : '<div class="empty">Aucune erreur GHL active.</div>')
  ...
  els.content.innerHTML = `
    <section class="mission-panel performance-hero">
      ...
      <div class="priority-strip">
        ...
        ${priorityTile("Envoyes sans reponse", sentNoResponse.length, "p3")}
        ${priorityTile("Relances dues", relanceDue.length, "p2")}
      </div>
      ${questionnaireFilters(responses, staleClients, sentNoResponse, relanceDue, sendErrors, clients)}
    </section>
    <section class="mission-panel performance-hero">
      <div class="section-head"><div><p class="eyebrow">A traiter</p><h2>${questionnaireFilterTitle(titleRows, staleClients)}</h2></div></div>
      ${primaryListHtml}
    </section>
    <section class="mission-panel">
    ...
    <section class="mission-panel">
      ...
      ${sentNoResponse.length ? sentNoResponse.slice(0, 8).map(questionnaireSendLogRow).join("") : '<div class="empty">Aucun questionnaire envoye sans reponse.</div>'}
    </section>
    <section class="mission-panel">
      <div class="section-head"><div><p class="eyebrow">Erreurs GHL</p><h2>A corriger avant envoi</h2></div></div>
      ${sendErrors.length ? sendErrors.slice(0, 8).map(questionnaireSendLogRow).join("") : '<div class="empty">Aucune erreur GHL active.</div>'}
    </section>
  `;
}
...
function questionnaireFilters(responses, staleClients, sentNoResponse, relanceDue, sendErrors, clients) {
  const activeResponses = responses.filter((row) => !isQuestionnaireArchived(row));
  const filters = [
    ["all", "Reponses recues", activeResponses.length],
    ...
    ["clients", "Envoyer", clients.length],
    ["stale", "A envoyer 3 mois+", staleClients.length],
    ["pending", "Envoyes sans reponse", sentNoResponse.length],
    ["relance", "Relance 7j+", relanceDue.length],
    ["errors", "Erreurs GHL", sendErrors.length],
    ["archive", "Archives", responses.filter((row) => ["lu", "action_completee", "archive"].includes(row.processingStatus)).length]
  ];
...
function questionnaireFilterTitle(filtered, staleClients) {
  if (state.activeQuestionnaireFilter === "clients") return "Envoyer un questionnaire";
  if (state.activeQuestionnaireFilter === "stale") return `${staleClients.length} questionnaire${staleClients.length > 1 ? "s" : ""} a envoyer`;
  if (state.activeQuestionnaireFilter === "pending") return `${filtered.length} envoye${filtered.length > 1 ? "s" : ""} sans reponse`;
  if (state.activeQuestionnaireFilter === "relance") return `${filtered.length} relance${filtered.length > 1 ? "s" : ""} due${filtered.length > 1 ? "s" : ""}`;
  if (state.activeQuestionnaireFilter === "errors") return `${filtered.length} erreur${filtered.length > 1 ? "s" : ""} GHL`;
...
function questionnaireSendLogRow(row) {
  const isError = row.statusKey === "erreur";
...
async function sendQuestionnaire(clientKey, clientName, taskId, button) {
  ...
  try {
    ...
    if (latest && latest.statusKey === "erreur") {
      showToast(`Questionnaire non envoye: ${latest.note || latest.status || "erreur GHL"}`, true);
...
async function updateRebookingStatus(payload, button) {
  ...
  showToast(payload.status === "OUVERT" ? "Rebooking reouvert localement. Synchronisation..." : "Rebooking masque des ouverts. Synchronisation...");
  render();
  try {
...
function applyLocalRebookingStatus(payload) {
  ...
  const nextStatus = payload.status === "OUVERT" ? "OUVERT" : "GERE";
  const matchRow = (row) => {
    if (!row) return false;
