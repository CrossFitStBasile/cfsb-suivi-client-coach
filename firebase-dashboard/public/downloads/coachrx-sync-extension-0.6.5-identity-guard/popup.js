...
async function fetchCoachRxDataInPage() {
  try {
    ...
    if (!identityApi || typeof identityApi.resolve !== "function" || typeof identityApi.summarize !== "function") {
    ...
    const apiData = await fetchCoachRxClientsFromApi(coachId, identityApi);
  ...
  async function fetchCoachRxClientsFromApi(coachId, identityApi) {
    ...
    const byId = new Map();
    const addClient = (client, index) => {
      const identity = identityApi.resolve(client);
      const fallback = `missing:${index}:${pickClientName(client).toLowerCase()}`;
      byId.set(identity.value || fallback, client);
    };
    best.clients.forEach((client, index) => addClient(client, index));
    for (let pageNumber = 1; pageNumber <= 10; pageNumber += 1) {
      ...
      pageClients.forEach((client, index) => addClient(client, `${pageNumber}:${index}`));
