...
Version actuelle: `0.6.8`. L'extension a une icone CFSB/Rx distincte pour eviter de la confondre avec les autres extensions Chrome.
...
La lecture API se fait dans la session CoachRx deja ouverte par le coach. L'extension
ne lit et ne transmet aucun cookie, jeton, `localStorage` ou `sessionStorage`; elle
transmet uniquement les donnees client structurees retournees par la route API du
coach dont l'identite est prouvee par l'URL.
...
9. Cliquer `Tester CoachRx` et verifier que `Identites stables` couvre tous les
   clients lus, puis cliquer `Synchroniser CoachRx`.
